...
private:
	...
	void cleanupSwapChain() 
	{
		...
		vkDestroyImageView(DeviceInfo.Device, renderer.depthImageView, nullptr);
		vkDestroyImage(DeviceInfo.Device, renderer.depthImage, nullptr);
		vkFreeMemory(DeviceInfo.Device, renderer.depthImageMemory, nullptr);

		for (auto framebuffer : renderer.swapChainFramebuffers) {
			vkDestroyFramebuffer(DeviceInfo.Device, framebuffer, nullptr);
		}

		vkFreeCommandBuffers(DeviceInfo.Device, renderer.commandPool, static_cast<uint32_t>(commandBuffers.size()), commandBuffers.data());

		vkDestroyPipeline(DeviceInfo.Device, renderer.graphicsPipeline, nullptr);
		vkDestroyPipelineLayout(DeviceInfo.Device, renderer.pipelineLayout, nullptr);
		vkDestroyRenderPass(DeviceInfo.Device, renderer.renderPass, nullptr);

		for (auto imageView : renderer.swapChainImageViews) {
			vkDestroyImageView(DeviceInfo.Device, imageView, nullptr);
		}

		vkDestroySwapchainKHR(DeviceInfo.Device, renderer.swapChain, nullptr);

		for (auto mesh : meshList)
		{
			for (size_t i = 0; i < renderer.swapChainImages.size(); i++) {
				vkDestroyBuffer(DeviceInfo.Device, mesh.uniformBuffers[i], nullptr);
				vkFreeMemory(DeviceInfo.Device, mesh.uniformBuffersMemory[i], nullptr);
			}

			vkDestroyDescriptorPool(DeviceInfo.Device, mesh.descriptorPool, nullptr);
		}
	}
	...
	void cleanup() 
	{
        ...
        vkDestroySampler(DeviceInfo.Device, texture.textureSampler, nullptr);
        vkDestroyImageView(DeviceInfo.Device, texture.textureImageView, nullptr);

        vkDestroyImage(DeviceInfo.Device, texture.textureImage, nullptr);
        vkFreeMemory(DeviceInfo.Device, texture.textureImageMemory, nullptr);
