...
private:
    ...
    VkImage depthtextImage;
    VkDeviceMemory depthtextImageMemory;
    VkImageView depthtextImageView;
    VkSampler depthtextSampler;
    ...
    glm::vec3 lightPos = glm::vec3(0.0, 0.0f, 1.0f);
    Camera camera;
    ...
    void mainLoop() {
        while (!glfwWindowShouldClose(window)) {
            ...
            ImGui::NewFrame();
            {
                ...
                ImGui::SliderFloat3("Light", &lightPos.x, -10.0f, 10.0f);
                ImGui::End();
    ...
    void createTextureImage() {
        ...
        stbi_uc* pixels = stbi_load("texture/brick_diffuseOriginal.bmp", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
    ...
    void createnormalTextureImage() {
        int texWidth, texHeight, texChannels;
        stbi_uc* pixels = stbi_load("texture/brick_normal.bmp", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
        VkDeviceSize imageSize = texWidth * texHeight * 4;
    ...
    void createdepthTextureImage() {
        ...
        createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthtextImage, depthtextImageMemory);

        transitionImageLayout(depthtextImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        copyBufferToImage(stagingBuffer, depthtextImage, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
        transitionImageLayout(depthtextImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    void createdepthTextureImageView() {
        depthtextImageView = createImageView(depthImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT);
    }
    ...
    void createdepthTextureSampler() {
        ...
        if (vkCreateSampler(device, &samplerInfo, nullptr, &depthtextSampler) != VK_SUCCESS) {
            throw std::runtime_error("failed to create texture sampler!");
        }
    }
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            ...
            VkDescriptorImageInfo depthtextimageInfo{};
            depthtextimageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
            depthtextimageInfo.imageView = depthtextImageView;
            depthtextimageInfo.sampler = depthtextSampler;
