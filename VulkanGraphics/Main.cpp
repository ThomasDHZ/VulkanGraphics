...
private:
    ...
    std::vector<VkDeviceMemory> quaduniformBuffersMemory;
    VkDescriptorPool quaddescriptorPool;
    std::vector<VkDescriptorSet> quaddescriptorSets;

    VkImage offscreenPasscolorimage;
    VkDeviceMemory offscreenPasscolorMemory;
    VkImageView offscreenPasscolorView;
    VkSampler offscreenPasscolorSampler;

    VkImage offscreenPassdepthimage;
    VkDeviceMemory offscreenPassdepthMemory;
    VkImageView offscreenPassdepthView;
    VkSampler offscreenPassdepthSampler;
    ...
    void initVulkan() {
        ...
        createTextureSampler();
        {
            ...
            createDescriptorSets(descriptorSets, descriptorPool);
        }
        {
            quaddescriptorSets.resize(swapChainImages.size());
            createVertexBuffer(quadvertices, quadvertexBuffer, quadvertexBufferMemory);
            createIndexBuffer(quadindices, quadindexBuffer, quadindexBufferMemory);
            createUniformBuffers(quaduniformBuffers, quaduniformBuffersMemory);
            createDescriptorPool(quaddescriptorPool);
            quadcreateDescriptorSets(quaddescriptorSets, quaddescriptorPool);
        }
    ...
    {
        {
            ...
            depthimage.extent.width = WIDTH;
            depthimage.extent.height = HEIGHT;
            ...
            if (vkCreateImage(device, &depthimage, nullptr, &offscreenPassdepthimage)) {
                throw std::runtime_error("failed to create vkCreateImage!");
            }
            vkGetImageMemoryRequirements(device, offscreenPassdepthimage, &memReqs);
            memAlloc.allocationSize = memReqs.size;
            memAlloc.memoryTypeIndex = findMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
            if (vkAllocateMemory(device, &memAlloc, nullptr, &offscreenPassdepthMemory)) {
                throw std::runtime_error("failed to create vkAllocateMemory!");
            }
            if (vkBindImageMemory(device, offscreenPassdepthimage, offscreenPassdepthMemory, 0)) {
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////
        {
            offscreenFramebuffers.resize(3);
            for (size_t i = 0; i < swapChainImageViews.size(); i++)
            {
                VkImageView attachments[2];
                attachments[0] = offscreenPasscolorView;
                attachments[1] = offscreenPassdepthView;
                ...
                if (vkCreateFramebuffer(device, &fbufCreateInfo, nullptr, &offscreenFramebuffers[i]))
                {
                    throw std::runtime_error("failed to create vkCreateImageView!");
                }
            }
    ...
    {
        {
            ...
            if (vkCreateImage(device, &image, nullptr, &offscreenPasscolorimage)) {
                throw std::runtime_error("failed to create vkCreateImage!");
            }
            vkGetImageMemoryRequirements(device, offscreenPasscolorimage, &memReqs);

            memAlloc.allocationSize = memReqs.size;
            memAlloc.memoryTypeIndex = findMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
            if (vkAllocateMemory(device, &memAlloc, nullptr, &offscreenPasscolorMemory)) {
                throw std::runtime_error("failed to create vkAllocateMemory!");
            }
            if (vkBindImageMemory(device, offscreenPasscolorimage, offscreenPasscolorMemory, 0)) {
                throw std::runtime_error("failed to create vkBindImageMemory!");
            }
            ...
            colorImageView.image = offscreenPasscolorimage;
            if (vkCreateImageView(device, &colorImageView, nullptr, &offscreenPasscolorView)) {
                throw std::runtime_error("failed to create vkCreateImageView!");
            }
            ...
            if (vkCreateSampler(device, &samplerInfo, nullptr, &offscreenPasscolorSampler)) {
    ...
    void createquadGraphicsPipeline() {
        ...
        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &quadgraphicsPipeline) != VK_SUCCESS) {
            throw std::runtime_error("failed to create graphics pipeline!");
        }
    ...
    void createVertexBuffer(std::vector<Vertex> vertexdata, VkBuffer& buffer, VkDeviceMemory& memory) {
        VkDeviceSize bufferSize = sizeof(vertexdata[0]) * vertices.size();
    ...
    void createIndexBuffer(std::vector<uint16_t> indicesdata, VkBuffer& buffer, VkDeviceMemory& memory) {
    ...
    void createUniformBuffers(std::vector<VkBuffer>& buffer, std::vector<VkDeviceMemory>& memory) {
    ...
    void createDescriptorSets(std::vector<VkDescriptorSet>& descriptorsets, VkDescriptorPool& descriptorpool) {
    ...
    void quadcreateDescriptorSets(std::vector<VkDescriptorSet>& descriptorsets, VkDescriptorPool& descriptorpool) {
        std::vector<VkDescriptorSetLayout> layouts(swapChainImages.size(), descriptorSetLayout);
        VkDescriptorSetAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
        allocInfo.descriptorPool = descriptorpool;
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++) {
            VkDescriptorBufferInfo bufferInfo{};
            bufferInfo.buffer = quaduniformBuffers[i];
            bufferInfo.offset = 0;
            bufferInfo.range = sizeof(UniformBufferObject);
        ...
        for (size_t i = 0; i < commandBuffers.size(); i++) {
            ...
            }
            {
                ...
                }
                {
                    ...
                    vkCmdBindVertexBuffers(commandBuffers[i], 0, 1, vertexBuffers, offsets);
                    vkCmdBindIndexBuffer(commandBuffers[i], indexBuffer, 0, VK_INDEX_TYPE_UINT16);
                    vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &quaddescriptorSets[i], 0, nullptr);
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        }
        {
            ...
            void* data;
            vkMapMemory(device, quaduniformBuffersMemory[currentImage], 0, sizeof(ubo), 0, &data);
            memcpy(data, &ubo, sizeof(ubo));
            vkUnmapMemory(device, quaduniformBuffersMemory[currentImage]);
        }
    }
