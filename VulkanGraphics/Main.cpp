...
private:
	...
	void initVulkan() {
		...
		MeshList.emplace_back(Model(DeviceInfo, swapChainExtent, renderPass, shaderInput, vertices2, indices, descriptorSetLayout));
		MeshList.emplace_back(Model(DeviceInfo, swapChainExtent, renderPass, shaderInput, vertices2, indices, descriptorSetLayout));
		MeshList.emplace_back(Model(DeviceInfo, swapChainExtent, renderPass, shaderInput, vertices2, indices, descriptorSetLayout));
		MeshList.emplace_back(Model(DeviceInfo, swapChainExtent, renderPass, shaderInput, vertices2, indices, descriptorSetLayout));
		MeshList.emplace_back(Model(DeviceInfo, swapChainExtent, renderPass, shaderInput, vertices2, indices, descriptorSetLayout));
	...
	void recreateSwapChain() {
		...
		for (auto mesh : MeshList)
		{
			mesh.RecreateSwapChainStage(swapChainExtent, renderPass, descriptorSetLayout, shaderInput);
		}
	...
	void createCommandBuffers() {
		...
		for (size_t i = 0; i < commandBuffers.size(); i++) {
			...
			for (auto mesh : MeshList)
			{
				mesh.Draw(commandBuffers[i], ShaderPipeline, ShaderPipelineLayout, i);
			}
