...
CubeTexture::CubeTexture(std::vector<std::string> CubeMapPaths, VkDevice device, VkPhysicalDevice physicalDevice, std::vector<VkCommandBuffer> commandBuffer, VkCommandPool commandPool, VkQueue graphicsQueue)
{
	Device = device;
	...
	LoadCubeMap(CubeMapPaths, device, physicalDevice, commandBuffer, commandPool, graphicsQueue);
...
void CubeTexture::LoadCubeMap(std::vector<std::string> CubeMapPaths, VkDevice device, VkPhysicalDevice physicalDevice, std::vector<VkCommandBuffer> commandBuffer, VkCommandPool commandPool, VkQueue graphicsQueue)
{
	std::vector<unsigned char> CubeMapData;
	int Channels;

	for (int x = 0; x < CubeMapPaths.size(); x++)
	{
		stbi_uc* Pixels = stbi_load(CubeMapPaths[x].c_str(), &Width, &Height, &Channels, STBI_rgb_alpha);
		ImageSize = Width * Height * STBI_rgb_alpha;
		CubeMapData.reserve(ImageSize * 6);

		auto LastByte = CubeMapData.size();
		CubeMapData.resize(CubeMapData.size() + ImageSize);
		...
		std::memcpy(&CubeMapData[LastByte], Pixels, ImageSize);
		stbi_image_free(Pixels);
	}
	ImageSize = CubeMapData.size();
	...
	memcpy(data, &CubeMapData[0], static_cast<size_t>(ImageSize));
...
void CubeTexture::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
...
void CubeTexture::CreateImage(VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties)
{
...
void CubeTexture::TransitionImageLayout(VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
{
...
void CubeTexture::CopyBufferToImage(VkBuffer buffer)
{
	VkDeviceSize SingleSize = ImageSize / 6;
	...
	for (int x = 0; x <= 5; x++)
	{
		...
		Region.bufferOffset = 0;
		RegionList.emplace_back(Region);
	}

	vkCmdCopyBufferToImage(CommandBuffer, buffer, TextureImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, static_cast<uint32_t>(RegionList.size()), RegionList.data());

	VulkanBufferManager::endSingleTimeCommands(Device, CommandBuffer, CommandPool, GraphicsQueue);
}

void CubeTexture::CreateImageView(VkFormat format, VkImageAspectFlags aspectFlags)
{
...
void CubeTexture::CreateTextureSampler()
{
...
void CubeTexture::CreateDescriptorSetLayout()
{
}

uint32_t CubeTexture::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
