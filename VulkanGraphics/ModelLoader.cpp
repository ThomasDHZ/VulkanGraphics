...
ModelLoader::ModelLoader(VulkanDevice deviceInfo, const std::string& FilePath)
{
...
void ModelLoader::ProcessNode(const std::string& FilePath, aiNode* node, const aiScene* scene)
{
	for (unsigned int i = 0; i < node->mNumMeshes; i++)
	{
		...
		NewMesh.TextureList = LoadTextures(FilePath, mesh, scene);
...
std::vector<unsigned int> ModelLoader::LoadIndices(aiMesh* mesh)
{
...
std::vector<Texture2D> ModelLoader::LoadTextures(const std::string& FilePath, aiMesh* mesh, const aiScene* scene)
{
