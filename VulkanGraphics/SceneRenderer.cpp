...
SceneRenderer::SceneRenderer(VulkanRenderer& renderer, std::shared_ptr<TextureManager>textureManager, std::shared_ptr<Texture>& texture)
{
    CreateRenderPass(renderer);
    DepthTexture = RendererDepthTexture(renderer);
    ColorTexture = std::make_shared<RendererColorTexture>(renderer);
...
void SceneRenderer::CreateRendererFramebuffers(VulkanRenderer& renderer, std::shared_ptr<RendererColorTexture> texture)
{
    SwapChainFramebuffers.resize(3);
    for (size_t i = 0; i < renderer.SwapChain.GetSwapChainImageCount(); i++)
    {
...
void SceneRenderer::UpdateSwapChain(VulkanRenderer& renderer, std::shared_ptr<RendererColorTexture> texture)
{
    ColorTexture->RecreateRendererTexture(renderer);
    BloomTexture->RecreateRendererTexture(renderer);
    DepthTexture.RecreateRendererTexture(renderer);
