...
Mesh::Mesh(Renderer& renderer, const std::vector<Vertex>& vertices, const std::vector<uint16_t>& indices, Texture2 textureList) : BaseMesh(renderer, vertices, indices)
{
	CreateUniformBuffers(renderer);
	CreateDescriptorPool(renderer);
	CreateDescriptorSets(renderer, textureList);
	CalcTangent();
}
...
Mesh::Mesh(Renderer& renderer, const std::vector<Vertex>& vertexList, const std::vector<uint16_t>& indexList, VkImageView RenderedTextureImageView, VkSampler RendereredTextureImageSampler) : BaseMesh(renderer, vertexList, indexList)
{
...
void Mesh::Draw(Renderer& renderer, int currentFrame)
{
	...
	{
		vkCmdBindPipeline(*GetRendererCommandBuffer(renderer, currentFrame), VK_PIPELINE_BIND_POINT_GRAPHICS, *GetMeshViewShaderPipeline(renderer));
		vkCmdBindVertexBuffers(*GetRendererCommandBuffer(renderer, currentFrame), 0, 1, vertexBuffers, offsets);
		vkCmdBindDescriptorSets(*GetRendererCommandBuffer(renderer, currentFrame), VK_PIPELINE_BIND_POINT_GRAPHICS, *GetShaderPipelineLayout(renderer), 0, 1, &descriptorSets[currentFrame], 0, nullptr);
		if (IndiceSize == 0)
		{
	...
	{
		vkCmdBindPipeline(*GetRendererCommandBuffer(renderer, currentFrame), VK_PIPELINE_BIND_POINT_GRAPHICS, *GetCollisionShaderPipeline(renderer));
		vkCmdBindVertexBuffers(*GetRendererCommandBuffer(renderer, currentFrame), 0, 1, vertexBuffers, offsets);
		vkCmdBindDescriptorSets(*GetRendererCommandBuffer(renderer, currentFrame), VK_PIPELINE_BIND_POINT_GRAPHICS, *GetShaderPipelineLayout(renderer), 0, 1, &descriptorSets[currentFrame], 0, nullptr);
		if (IndiceSize == 0)
		{
	...
	vkCmdBindPipeline(*GetRendererCommandBuffer(renderer, currentFrame), VK_PIPELINE_BIND_POINT_GRAPHICS, *GetShaderPipeline(renderer));
	vkCmdBindVertexBuffers(*GetRendererCommandBuffer(renderer, currentFrame), 0, 1, vertexBuffers, offsets);
	vkCmdBindDescriptorSets(*GetRendererCommandBuffer(renderer, currentFrame), VK_PIPELINE_BIND_POINT_GRAPHICS, *GetShaderPipelineLayout(renderer), 0, 1, &descriptorSets[currentFrame], 0, nullptr);
	if(IndiceSize == 0)
	{
...
void Mesh::UpdateDescriptorSets(Renderer& renderer)
{
	VkDescriptorImageInfo DiffuseMap = {};
	DiffuseMap.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
	DiffuseMap.imageView = TextureList.DiffuseMap.textureImageView;
